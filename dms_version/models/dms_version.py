...
class DmsVersion(models.Model):
    ...
    @api.model
    def clean_versions(self, dms_files, max_size=None):
        for dms_file in dms_files.filtered(lambda rec: rec.count_versions > 0):
            max_size = max_size or dms_file.storage_id.stored_versions
            if dms_file.count_versions > max_size:
                remove_size = dms_file.count_versions - max_size
                remove_versions = self.env["dms.version"]
                current = dms_file.version_ids.filtered(
                    lambda rec: not rec.previous_version
                )
    ...
    @api.model
    def add_version(self, dms_file, content):
        domain = [
            ("file_id", "=", dms_file.id),
            ("next_version", "=", False),
        ]
        vals = {"name": dms_file.name, "file_id": dms_file.id, "content": content}
        record = self.search(domain, limit=1)
        if not record:
            return self.create(vals)
        vals["previous_version"] = record.id
        new_version = self.create(vals)
        record.write({"next_version": new_version.id})
        return new_version

    @api.model
    def pop_version(self, dms_file):
        domain = [
            ("file_id", "=", dms_file.id),
            ("next_version", "=", False),
        ]
